...
exports.verifyEmail = asyncHandler (async  (req, res, next) => {
    ...
    const resetURL = req.protocol + '://' + req.get('host') + '/api/v1/users/verifyEmail/' + resetToken;

    const message = 'Verify your account \nthe link for verify: ' + resetURL + '.\nIf you verified  your account, please ignore this email!';
    ...
    try {
        ...
        await UserRepo.UpdateStatus(user.id)
        return new SuccessMsgResponse('Token sent to email!').send(res);
    } catch (err) {
        await user.save({ validateBeforeSave: false });
